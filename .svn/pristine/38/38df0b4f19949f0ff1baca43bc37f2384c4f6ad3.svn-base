...
public class UploadBaseAction {
	...
	@Resource
	protected FileStoreFacade fileStoreFacade;
	
	private String allowSuffix = "jpg,png,gif,jpeg,zip";//允许文件格式
    private long allowSize = 2000L;//允许文件大小
    ...
    public void upload(MultipartFile file, String destDir,HttpServletRequest request) throws Exception {
        try {
                ...
                fileStoreVO.setFileName(new String(file.getOriginalFilename().getBytes("utf-8")));
                fileStoreVO.setSuffix(suffix);
                fileStoreVO.setFilePath(realPath+destDir+File.separatorChar+fileNameNew);
                fileStoreVO.setUrl("/upload/user/"+fileNameNew);
                fileStoreVO.setType(file.getContentType());
                fileStoreVO.setSize(Double.valueOf(file.getSize()));
