...
public class DefaultAuthenticationSuccessHandler extends
	...
	public void onAuthenticationSuccess(HttpServletRequest request,
		...
		this.saveMemberInfo(request,authentication);
		
		String type = request.getParameter("type");
		//前台登录
		if(null != type && type.equalsIgnoreCase("1") && 
				request.getSession().getAttribute("memberInfo") == null){
			response.setCharacterEncoding("UTF-8");
            response.getWriter().print("{\"flag\":0,\"data\":{\"error\":\"会员信息为空\",\"code\":\"-1\"}}");
            return;
		}
	 ...
	 * @param authentication
	 */
	private void saveMemberInfo(HttpServletRequest request,
			Authentication authentication) {
		String stype = request.getParameter("type");
		if(StringUtils.isNotEmpty(stype) && stype.equalsIgnoreCase("1")){//前台会员登录
			SysUserVO user = (SysUserVO) authentication.getPrincipal();
			MemberVO mvo = memberDao.getMember(new MemberVO(user.getAccount()));
			if(mvo == null){
				logger.error("根据系统帐号查询会员为null,帐号:"+user.getAccount());
			}
