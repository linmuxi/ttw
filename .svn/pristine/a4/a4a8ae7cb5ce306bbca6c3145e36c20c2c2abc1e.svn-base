	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			dragStart: function (dragtype, e, obj) { //default drag function start
				...
				} else if (dragtype == 'colMove' && false) {//column header drag
					$(g.nDiv).hide();
					$(g.nBtn).hide();
			...
			addData: function (data) { //parse data
				if (p.dataType == 'json') {
					data = $.extend({rows: [], current: 0, total: 0}, data);
				}
				if (p.preProcess) {
					...
					$.each(data.rows, function (i, row) {
						...
						$('thead tr:first th', g.hDiv).each(
							function () {
								...
								*/
								if(typeof p.colModel[idx].name != "undefined"){
									if(typeof p.colModel[idx].formatter != "undefined"){
										//td.innerHTML = row[p.colModel[idx].name] != 'null' ? row[p.colModel[idx].name] : '';
										var formatterFun = p.colModel[idx].formatter;
										td.innerHTML = formatterFun(p.colModel[idx].name, row);
									}else{
										td.innerHTML = row[p.colModel[idx].name] != 'null' ? row[p.colModel[idx].name] : '';
									}
								}
								//alert(row[p.colModel[idx].name]);
			...
			buildpager: function () { //rebuild pager based on new properties
				$('.pcontrol input', this.pDiv).val(p.current);
				$('.pcontrol span', this.pDiv).html(p.pages);
				var r1 = (p.current - 1) * p.rp + 1;
				var r2 = r1 + p.rp - 1;
				if (p.total < r2) {
					r2 = p.total;
				}
         		...
         		$('tr', g.hDiv).each(
                     ...
                     cthch.click(function () {
                         if (this.checked) {
                             $('tbody tr', g.bDiv).each(function () {
                            	 //判断是否存在checkbox
                            	 if($(this).children('th').eq(0).find('input')[0]){
